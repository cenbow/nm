...
var DateUtil = function() {
	...
	this.dateTimeFormat = function(date) {
		return this.dateFormat(date,"yyyy-MM-dd HH:mm:ss");
	}
	
	this.dateMinuteFormat = function(date) {
		
		var oDate = this.dateFormat(date,"yy-MM-dd HH:mm");
		
		var sttr = oDate.substring(9);
		
		var diffentDay =this.dateDiff('d', date, new Date().getTime());
	 ...
	 * @param f 日期格式,格式定义如下 yyyy-MM-dd
	 ...
	 */
	this.dateFormat = function(date,formatStr) {
		date = arguments[0];
