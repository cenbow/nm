...
public class ValidationUtil {
	...
	public static String formatDate(Object input) {
		String inputStr = "";
		inputStr = (String) input;
		if (inputStr != null && !inputStr.equals("") && !inputStr.equals("null") && inputStr.length() > 10) {
	...
	public static boolean isNotBlank(Object object) {
		try {
			if (object == null) {
				return false;
			} else if ("null".equals(object) || "null".equals(String.valueOf(object).trim())) {
				return false;
			} else {
	...
	public static long dealNullLong(Object obj, int i) {
		...
		try {
			...
			System.out.println("ValidationUtil.dealNullLong the null" + ex.getMessage());
	...
	public static String cutDouble2(String doubleStr) {
		...
		try {
			...
			System.out.println("ValidationUtili.cutDouble2错误-->" + ex.getMessage());
			// ex.printStackTrace();
		}
		return result;
	}
	...
	public static double coverDouble(String strLong, double i) {
		...
		try {
			if (strLong != null) {
				if (strLong.trim().equals(""))
					strLong = "0";
				values = Double.parseDouble(strLong);
			}
		} catch (Exception ex) {
			System.out.println("coverDouble:" + ex.getMessage());
			values = i;
		}
		return values;
	}
